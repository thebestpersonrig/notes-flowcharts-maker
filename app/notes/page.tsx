...
export default function Home() {
  ...
  // ─── Effects ───────────────────────────────────────────────────────────────
  useEffect(() => { const s = localStorage.getItem("learnix-theme") || "dark"; setTheme(s as "dark"|"light"); document.documentElement.classList.toggle("dark", s === "dark"); }, []);
  useEffect(() => { document.documentElement.classList.toggle("dark", theme === "dark"); localStorage.setItem("learnix-theme", theme); }, [theme]);
  useEffect(() => { try { setHistory(JSON.parse(localStorage.getItem("learnix-history") || "[]")); } catch { /* */ } }, []);
  ...
  const saveToHistory = useCallback((n: NotesContent) => {
    ...
    setHistory(prev => { const u = [item, ...prev].slice(0, 20); localStorage.setItem("learnix-history", JSON.stringify(u)); return u; });
  }, []);
  ...
  const deleteFromHistory = (id: string) => { setHistory(prev => { const u = prev.filter(h => h.id !== id); localStorage.setItem("learnix-history", JSON.stringify(u)); return u; }); };
  ...
  async function sendChatMessage(message: string) {
    ...
    try { const r = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: msg, notesContext: notes, grade: grade || undefined }) }); let d; try { d = await r.json(); } catch { throw new Error("Invalid response from AI"); } if (!r.ok) throw new Error(d.error); setChatMessages(prev => [...prev, { role: "assistant", content: d.reply }]); }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0a0a1a] text-slate-800 dark:text-slate-200 transition-colors duration-300 relative">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-16 relative z-10">
        ...
        {/* ─── Templates ─────────────────────────────────────── */}
        {!notes && !loading && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mb-5 stagger-children">
            {TEMPLATES.map(t => (
              <button key={t.id} type="button" onClick={() => selectTemplate(t)} className={`flex flex-col items-center gap-1.5 p-4 rounded-2xl border transition-all duration-200 ${template === t.id ? "border-indigo-500/50 bg-indigo-500/10 glow-violet scale-[1.02]" : "glass hover:bg-white/10 hover:scale-[1.01]"}`}>
                ...
                <span className="text-[11px] text-slate-500 dark:text-slate-400 hidden sm:block leading-tight text-center">{t.desc}</span>
              </button>
            ))}
          </div>
        )}
          ...
          <form onSubmit={handleGenerate} className={`glass-strong rounded-2xl p-5 sm:p-6 mb-8 no-print animate-fadeInUp ${notes ? "glow-sm" : ""}`} style={!notes ? { animationDelay: "200ms" } : undefined}>
            <div className="space-y-3">
              ...
              <div className="flex flex-col gap-3 pt-1">
                {/* Options row — responsive grid on mobile */}
                <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2.5 items-center">
                  <button type="button" onClick={addTopic} className="text-indigo-500 dark:text-indigo-400 text-sm font-medium hover:text-indigo-400 transition flex items-center gap-1 col-span-2 sm:col-span-1 justify-center sm:justify-start">
                  ...
                  <div className="flex rounded-xl border border-white/10 overflow-hidden bg-white/5 col-span-2 sm:col-span-1">
                    {([
                      ...
                      <button key={opt.value} type="button" onClick={() => setLength(opt.value)}
                        className={`flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-3.5 py-2 text-sm font-medium transition-all ${length === opt.value ? "bg-indigo-500/20 text-indigo-400 shadow-inner" : "text-slate-500 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 hover:bg-white/5"}`}>
      ...
      {/* ─── Chat Sidebar ─────────────────────────────────── */}
      {chatOpen && notes && (<>
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40 animate-fadeIn" onClick={() => setChatOpen(false)} />
        <div className="fixed inset-y-0 right-0 w-full sm:w-96 glass-strong z-50 flex flex-col shadow-2xl animate-slideInRight sm:rounded-l-2xl">
          <div className="flex items-center justify-between p-5 border-b border-white/10">
            <h3 className="font-semibold text-slate-900 dark:text-white flex items-center gap-2">
              ...
              Ask AI about {notes.title.length > 20 ? notes.title.slice(0, 20) + "…" : notes.title}
            </h3>
            <button onClick={() => setChatOpen(false)} className="p-1.5 rounded-lg hover:bg-white/10 text-slate-400 hover:text-white transition">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {chatMessages.length === 0 && (
              <div className="text-center py-12">
                <div className="w-14 h-14 mx-auto mb-4 rounded-2xl bg-violet-500/15 flex items-center justify-center">
                  <svg className="w-7 h-7 text-violet-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
                </div>
                <p className="text-slate-400 text-sm mb-4">Ask anything about your notes</p>
                <div className="space-y-2">
                  {["Explain this simpler", "Give me more examples", "How would this be on a test?"].map(q => (
                    <button key={q} onClick={() => sendChatMessage(q)} className="w-full text-left px-4 py-2.5 rounded-xl text-sm text-slate-300 bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/10 transition">
                      {q}
                    </button>
                  ))}
                </div>
            ...
            {chatMessages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${msg.role === "user" ? "bg-indigo-500 text-white rounded-br-md" : "bg-white/8 text-slate-300 border border-white/5 rounded-bl-md"}`}>
          ...
          <form onSubmit={handleChatSubmit} className="p-4 border-t border-white/10">
            <div className="flex gap-2">
              <input value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Ask a question..." disabled={chatLoading}
                className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500/40 transition" />
              <button type="submit" disabled={chatLoading || !chatInput.trim()}
                className="px-4 py-3 rounded-xl bg-violet-500 text-white font-medium text-sm disabled:opacity-40 hover:bg-violet-600 transition active:scale-95">
